...
namespace FluentFTP {
	public partial class AsyncFtpClient {
		...
		/// </returns>
		public async Task<List<FtpResult>> DownloadDirectory(string localFolder, string remoteFolder, FtpDownloadDirectoryConfig config, IProgress<FtpProgress> progress = null, CancellationToken token = default(CancellationToken)) {
			...
			var listing = await GetListing(remoteFolder, lOpt, token);
			...
			await DownloadServerFilesAsync(toDownload, config.ExistsMode, config.VerifyOptions, progress, token);
